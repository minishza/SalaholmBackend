...
public class PrayerMapper {
    ...
    public Prayers toPrayers(List<String> prayerData, int month) {
    ...
    private List<PrayerTime> buildPrayerTimes(List<String> prayerData, Prayers prayer) {
        List<PrayerTime> prayerTimes = new ArrayList<>();
        for (int prayerOrder = 1; prayerOrder < prayerData.size(); prayerOrder++) {
            prayerTimes.add(
                    buildPrayerTimeFromString(prayerData.get(prayerOrder))
                            .prayerName(PrayerName.fromIndex(prayerOrder)
                                    .orElseThrow(() -> new IllegalArgumentException("Invalid enum prayer index")))
                            .prayers(prayer)
                            .build()
            );
        }
